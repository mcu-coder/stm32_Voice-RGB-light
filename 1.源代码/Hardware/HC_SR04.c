...
void HC_SR04_Ranging(u16 *data)
{
	...
	u32 Count = 0;
	static u32 i = 0;
	while(GPIO_ReadInputDataBit(HC_ECHO_PROT,ECHO) == 1);
	TRIG_HIGH;
	...
	if(HC_SR04_DelayStructure.delay20msFinishFlag)
	{
		...
		while(GPIO_ReadInputDataBit(HC_ECHO_PROT,ECHO) == 0);
